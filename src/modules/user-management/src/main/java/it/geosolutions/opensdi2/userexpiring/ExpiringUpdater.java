...
public class ExpiringUpdater {
	 ...
	 */
	SimpleDateFormat formatter = new SimpleDateFormat(this.dateFormat);// e.g.
																		// dd-MMM-yyyy
	...
	/**
	 * Check user list and found candidate users for expiring. if expired, set
	 * user enabled to false.
	 * 
	 */
	public ArrayList<User> checkUserList() {
		...
		if (l != null && l.getList() != null) {
			LOGGER.debug("found " + l.getList().size() + " users on GeoStore");
	...
	/**
	 * disable user using GeoStore client. Query geostore and check expiring
	 * date for each user. Disable them if the date is past
	 * 
	 * @param u
	 *            the user to disable
	 */
