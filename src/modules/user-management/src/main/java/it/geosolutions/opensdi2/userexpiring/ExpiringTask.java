...
import it.geosolutions.opensdi2.userexpiring.util.Util;
...
/**
 * Class that wraps the method execution for expiring users check.
 * Send email if configured to do it
 * 
...
public class ExpiringTask {
	...
	/**
	 * email from address 
	 * (note: gmail or other services could not allow to use an address different from the account)
	 * 
	 */
	...
	/**
	 * this method check if the program is configured to expire users
	 * and then run the process. Can be schedule
	 */
	public void execute(){
		if(autoExpireUsers){
	...
	/**
	 * This method check user expiring and notify
	 * the administrator
	 */
	public void runExpiringUsers() {
		...
		synchronized(userExpiringTaskStatus){
			if(userExpiringTaskStatus.getStatus()==TaskState.Running){
				userExpiringTaskStatus.setMessage("task was running at " + new Date() +" while trying to execute the process again");
				return;
		...
		if(!pingGeoStore()){
			...
			synchronized(userExpiringTaskStatus){
				...
				userExpiringTaskStatus.setMessage("GeoStore is unreachable at:"+ administratorGeoStoreClient.getGeostoreRestUrl());
	...
	/**
	 * utility method for to get a rest user from a user
	 ...
	 * @param expired the list of expired users
     * @param logEl the log element to fill
	 * @return false if some error occurred, true otherwise
	 */
	private boolean notifyAdminUserExpiring(List<User> expired, ExpireLogElement logEl) {
		...
		try {
			//create message
			String message = notificationService.applyTemplate("adminExpireNotification.ftl", model);
			LOGGER.info("Sending email to:" + notifyTo );
			//sending email
			notificationService.sendMail(mailFrom, notifyTo, checksumSubject, message);
        	...
        	notification.setStatus(ExecutionState.Error);
        	notification.setMessage("Error sending expire checksum email to " + notifyTo);
			logEl.getEmailNotificationLog().add(notification);
        	return false;
        }
		return true;
	 ...
	 * @return false if some error occurred, true otherwise
	 */
	private boolean notifyUsersAccountExpiring(List<User> expired,ExpireLogElement logEl) {
		boolean ret = true;
	...
	/**
	 * Send email to the user. The user must have a field 
	 * defined as email
	 * @param user the user to notify
	 * @param the template to use
	 * @param logEl the log element to fill
	 * @return false if some error occurred, true otherwise
	 */
	private boolean sendMailToUser(User user,String template, ExpireLogElement logEl){
		Map<String,Object> model = new HashMap<String,Object>();
		NotificationLogElement notification = new NotificationLogElement();
	...
	public void setUserExpiringTaskStatus(UserExpiringStatus userExpiringTaskStatus) {
