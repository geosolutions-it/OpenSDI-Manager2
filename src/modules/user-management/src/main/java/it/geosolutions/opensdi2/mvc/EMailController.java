...
/**
 * Class that allows to create and send email for users or generic from the
 * selected templates
 * 
...
public class EMailController {
	...
	/**
	 * Apply the template for an email using the user in the template model
	 * 
	 * @param template
	 *            the template name
	 * @param username
	 *            the name of the user to use for the model
	 * @return the message body
	 */
	@RequestMapping(value = "/user/{template}/{username}/body", method = RequestMethod.GET)
	public @ResponseBody
	String getMessageBody(@PathVariable String template,
		...
		try {
			...
			messageBody = "could't get the template:" + template + ". "
					+ e.getMessage();
			LOGGER.error(messageBody);
		} catch (TemplateException e) {
			// TODO error response to improve
			messageBody = "could't generate the body message from the template:"
